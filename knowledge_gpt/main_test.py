...
if st.button("Synthesize All Transcripts"):
    # Collect all answers from previous responses
    all_answers = "\n".join(
        response['answer'] for response in st.session_state['responses_and_sources']
    )

    # Use the combined answers as input for summarization
    if all_answers.strip():
        openai_model = OPENAI_MODEL_MAPPING.get(model)
        if openai_model is None:
            st.error(f"Model {model} is not supported.")
        else:
            # Call the synthesize_insights function to get the summary
            summary = synthesize_insights(all_answers, openai_api_key, openai_model)
            st.markdown("### Synthesized Insights")
            st.markdown(summary)
            st.session_state['previous_responses'].append({'answer': summary, 'sources': []})
    else:
        st.error("No answers available to synthesize.")
